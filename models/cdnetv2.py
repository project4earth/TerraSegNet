...
class CDNetV2_MODEL(nn.Module):
    def __init__(self, block, layers, in_channels, out_channels, aux=False):
        self.inplanes = 256 
        self.aux = aux		
        super().__init__()
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1):
        ...
        for i in downsample._modules['1'].parameters(): # type: ignore
            i.requires_grad = False
        layers = []
    ...
    def forward(self, x):
        ...
        output = F.interpolate(output, x.shape[-2:], mode='bilinear', align_corners=True)
...
def CDNetV2(in_channels, out_channels):
    model = CDNetV2_MODEL(Bottleneck, [3, 4, 6, 3], in_channels=in_channels, out_channels=out_channels)
    return model	