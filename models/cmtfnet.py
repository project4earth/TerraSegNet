...
class ResNet18(nn.Module):
    def __init__(self, in_channels):
        ...
        if in_channels != 3:
            ...
            with torch.no_grad():
                if in_channels < 3:
                    self.conv1.weight[:, :in_channels] = old_conv.weight[:, :in_channels]
                    self.conv1.weight[:, in_channels:] = old_conv.weight[:, :1] 
                else:
                    self.conv1.weight[:, :3] = old_conv.weight
                    if in_channels > 3:
                        for i in range(3, in_channels):
                            self.conv1.weight[:, i] = old_conv.weight[:, i % 3]
        else:
            self.conv1 = old_conv  
...
class MutilScal(nn.Module):
    def __init__(self, dim=128, fc_ratio=4, dilation=[3, 5, 7], pool_ratio=16):
...
class Block(nn.Module):
    def __init__(self, dim=128, num_heads=16,  mlp_ratio=4, pool_ratio=16, drop=0., dilation=[3, 5, 7],
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.b4 = Block(dim=decode_channels, num_heads=8, mlp_ratio=4, pool_ratio=16, dilation=dilation[0])

        self.p3 = Fusion(decode_channels)
        self.b3 = Block(dim=decode_channels, num_heads=8, mlp_ratio=4, pool_ratio=16, dilation=dilation[1])

        self.p2 = Fusion(decode_channels)
        self.b2 = Block(dim=decode_channels, num_heads=8, mlp_ratio=4, pool_ratio=16, dilation=dilation[2])
...
class CMTFNet(nn.Module):
    def __init__(self,
        ...
        self.decoder = Decoder(num_classes, encode_channels, decode_channels, dropout=dropout)
