...
class HRNet(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        num_channels = [32, 64]
        block = blocks_dict['BASIC']
        num_channels2 = [num_channels[i] * block.expansion for i in range(len(num_channels))]
        self.transition1 = self._make_transition_layer([stage1_out_channel], num_channels2)
        self.stage2, pre_stage_channels = self._make_stage(num_channels2, num_channels)

        num_channels = [32, 64, 128]
        block = blocks_dict['BASIC']
        num_channels2 = [num_channels[i] * block.expansion for i in range(len(num_channels))]
        self.transition2 = self._make_transition_layer(pre_stage_channels, num_channels2)
        self.stage3, pre_stage_channels = self._make_stage(num_channels2, num_channels)

        num_channels = [32, 64, 128, 256]
        block = blocks_dict['BASIC']
        num_channels2 = [num_channels[i] * block.expansion for i in range(len(num_channels))]
        self.transition3 = self._make_transition_layer(pre_stage_channels, num_channels2)
        self.stage4, pre_stage_channels = self._make_stage(num_channels2, num_channels, multi_scale_output=True)
        ...
        self.last_layer = nn.Sequential(
            ...
            nn.ReLU(inplace=relu_inplace),
            nn.Conv2d(in_channels=last_inp_channels, out_channels=out_channels, kernel_size=1, stride=1)
        )
    ...
    def _make_stage(self, num_inchannels, num_channels, multi_scale_output=True):
        num_modules = 1
        num_branches = len(num_inchannels)
        num_blocks = [4] * len(num_channels)
